        ...
        <Link href={`/blog/${encodeURIComponent(props.slug.current)}`} passHref={true}>
            <a className="cursor-pointer flex flex-row w-full group">
                ...
                <div className="flex flex-col bg-black/0 w-full pb-2.5 group-hover:bg-black/20 transition-colors">
                    <div className="flex flex-row justify-between w-full bg-gray-900/80 px-1.5 py-0.5 group-hover:bg-brown-800 transition-colors">
                        <div className="flex flex-row shrink w-11/12">
                            ...
                            <p className="w-full break-all line-clamp-1 text-main-100 group-hover:text-gray-250">{props.title}</p>
                        </div>
                        <p className="hidden md:block main-300 text-main-100 group-hover:text-gray-250">{`${new Date(props.publishedDate).toLocaleDateString()}`}</p>
                    </div>
                    <div className="flex flex-col mt-2 ml-4 ">
                        <Badge>{props.tag}</Badge>
                        <p className="block md:hidden mr-2 text-xs text-main-300 group-hover:text-gray-250">{`${new Date(props.publishedDate).toLocaleDateString()}`}</p>
                    </div>
